...
# import tome
# from model.tome import apply_patch
# from model.ltmp import inference_ltmp_deit_small_patch16_224
...
parser.add_argument(
    "--device", default="cuda", type=str, help="device to run benchmark on"
)
...
parser.add_argument(
    "--patch_merge", default=False, action="store_true", help="enable patch merging"
)
parser.add_argument(
    "--merge_method", default="keep1", help="the method to merge patches"
)
parser.add_argument(
    "--merge_group_num", default=0, type=int, help="the number of patches to be merged"
)
parser.add_argument(
    "--merge_group_size",
    default=[1, 2],
    ...
    type=int,
    metavar="N N",
    help="the group size to merge patches",
)
parser.add_argument(
    "--distance",
    default="manhattan",
    help="the method to measure similarity of patches",
)
parser.add_argument(
    "--token_pos",
    default=[0, 0],
    ...
    type=int,
    metavar="N N",
    help="the position of reference token in a group",
)
parser.add_argument("--score_gate", default=None, help="the score to filter patches")
...
class BenchmarkRunner:
    def __init__(
        ...
        **kwargs,
        ...
        if fuser:
            set_jit_fuser(fuser)
        # if kwargs["token_num"] != None:
        #     kwargs["model_kwargs"] = {
        #         "token_num": kwargs["token_num"],
        #         "merge_depth": kwargs["merge_depth"],
        #     }
        ...
        if torchscript:
            ...
            assert (
                has_compile
            ), "A version of torch w/ torch.compile() is required, possibly a nightly."
            torch._dynamo.reset()
    ...
    def run(self):
        ...
        with torch.no_grad():
            ...
            for i in range(self.num_bench_iter):
                ...
                num_steps = i + 1
                max_memory = torch.cuda.max_memory_reserved(self.device) / 1e9
        ...
        while retries:
            ...
            try:
                if has_deepspeed_profiling:
                    ...
                    macs, activations = profile_fvcore(
                        self.model, self.input_size, force_cpu=not retries
                    )
                    results["gmacs"] = round(macs / 1e9, 2)
        ...
        _logger.info(
            ...
            f"Occupy memory {results['max_memory']:.2f} GB"
        )
...
class TrainBenchmarkRunner(BenchmarkRunner):
    def __init__(self, model_name, device="cuda", torchscript=False, **kwargs):
        ...
        self.optimizer = create_optimizer_v2(
            self.model, opt=kwargs.pop("opt", "sgd"), lr=kwargs.pop("lr", 1e-4)
        )
...
class ProfileRunner(BenchmarkRunner):
    def __init__(self, model_name, device="cuda", profiler="", **kwargs):
        ...
        assert (
            profiler
        ), "One of deepspeed or fvcore needs to be installed for profiling to work."
    ...
    def run(self):
        ...
        if self.profiler == "deepspeed":
            macs, _ = profile_deepspeed(
                self.model, self.input_size, batch_size=self.batch_size, detailed=True
            )
    ...
    while batch_size:
        try:
            ...
            if not check_batch_size_retry(error_str):
                _logger.error(
                    f"Unrecoverable error encountered while benchmarking {model_name}, skipping."
                )
                break
...
def benchmark(args):
    if args.amp:
        ...
        args.precision = (
            "amp" if args.amp_dtype == "float16" else "_".join(["amp", args.amp_dtype])
        )
    _logger.info(
    ...
    if bench_kwargs["patch_merge"]:
        bench_kwargs["model_kwargs"] = {
            "merge_method": args.merge_method,
            "merge_group_num": args.merge_group_num,
